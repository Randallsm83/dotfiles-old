...
local function get_process_icon(process)
  return PROCESS_ICONS[process:lower()] or ICONS.default_process
end
...
wez.on("format-tab-title", function(tab, _, _, conf)
  local process = tab.active_pane.foreground_process_name or "term"
  ...
  if not tab.is_active and tab.has_unseen_output then
    formatted_title = "wezterm.nerdfonts.cod_circled_filled " .. formatted_title
  end
